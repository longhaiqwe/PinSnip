...
final class UpdateConfigurationTests: XCTestCase {
    ...
    func testAppcastPublishesCurrentSignedVersion() throws {
        ...
        XCTAssertTrue(appcast.contains("<sparkle:version>11</sparkle:version>"))
        XCTAssertTrue(
            appcast.contains(
        ...
        XCTAssertTrue(
            appcast.contains(
                "releases/download/v0.7.3/PinSnip-v0.7.3-macOS-universal.zip"
            )
        )
        ...
        XCTAssertTrue(
            appcast.contains(
                "<sparkle:fullReleaseNotesLink>"
                    + "https://github.com/longhaiqwe/PinSnip/releases/tag/v0.7.3"
                    + "</sparkle:fullReleaseNotesLink>"
            )
        )
        XCTAssertFalse(
            appcast.contains(
                "raw.githubusercontent.com/longhaiqwe/PinSnip/main/"
                    + "PinSnip-v0.7.3-macOS-universal.md"
            )
        )
    }
}
