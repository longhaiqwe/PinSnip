...
- `F3` 框选屏幕区域录制 GIF；包含鼠标，最长 30 秒，自动增加取自首帧的留白、圆角边框与阴影，停止后复制、保存并预览（可配置）
- `F4` 将剪贴板图像、图片文件、文字或 HEX 颜色贴到屏幕（可配置）
- 菜单栏可录制并持久化截图、动图录制、贴图快捷键
...
5. 剪贴板中已有内容时，按 `F4` 可直接创建贴图。
6. 需要录制屏幕操作时，按 `F3` 或从菜单栏选择“录制动图…”，框选区域后点击红色录制按钮；完成后在浮动控制条点击“停止并生成 GIF”。
